...
function gridos_my_load_more_scripts() {
 global $query; 
 $query = new WP_Query();
 ...
 wp_register_script( 'gridos_loadmore', get_stylesheet_directory_uri() . '/assets/js/myloadmore.js', array('jquery') );

 // now the most interesting part
 // we have to pass parameters to myloadmore.js script but we can get the parameters values only in PHP
 // you can define variables directly in your HTML but I decided that the most proper way is wp_localize_script()
 wp_localize_script( 'gridos_loadmore', 'gridos_loadmore_params', array(
  ...
  wp_enqueue_script( 'gridos_loadmore' );
}

add_action( 'wp_enqueue_scripts', 'gridos_my_load_more_scripts' );

function gridos_loadmore_ajax_handler(){
...
add_action('wp_ajax_loadmore', 'gridos_loadmore_ajax_handler'); // wp_ajax_{action}
add_action('wp_ajax_nopriv_loadmore', 'gridos_loadmore_ajax_handler'); // wp_ajax_nopriv_{action}